...
- `dist/index.html` includes the stylesheet, `main.js`, and the SVG icons. It is about 10 KB gzipped, small enough to arrive in the first round trip after the connection opens. The build fails if a referenced file changes shape and can no longer be inlined. The separate files are still published for pages cached before a deploy.
